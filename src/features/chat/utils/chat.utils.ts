 ...
 */
export class ChatFilters {
  /**
   * Filter messages by date range
   */
  static filterByDateRange(messages: Message[], startDate: Date, endDate: Date): Message[] {
   ...
   */
  static filterByRole(messages: Message[], role: 'user' | 'assistant'): Message[] {
    return messages.filter(msg => msg.role === role);
  }

  /**
   * Filter messages with errors
   */
  static filterErrorMessages(messages: Message[]): Message[] {
   ...
   */
  static filterUnsyncedMessages(messages: Message[]): Message[] {
   ...
   */
  static getTodaysMessages(messages: Message[]): Message[] {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    
    return this.filterByDateRange(messages, today, tomorrow);
  }

  /**
   * Get messages from last N days
   */
  static getRecentMessages(messages: Message[], days: number): Message[] {
